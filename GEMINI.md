...
- 执行与后续：
  ...
  - 严格按 `AI_actions` 落地：包括产物存在/非空校验、快速审阅关键字段、继续后续自动化（如将“产物路径”传入下一指令）、异常处理与重试建议。
- 失败与回退：
...
- 示例（意图→匹配→执行→后续）：
  ...
  - 执行：`npm run sum:jira CS-123456`
  - 后续：读取返回的“提示词文件路径”，用 `test -s <路径>` 校验非空；审阅并按 `AI_actions` 继续（例如把文件路径传入后续自动化指令或直接用于生成结果）。
